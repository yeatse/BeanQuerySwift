---
title: Beancount Query Language (BQL) Guide
source: Beancount - Query Language.docx (Martin Blais, January 2015)
purpose: Provide an AI coding agent with a practical reference to BQL (bean-query) syntax and semantics.
---
...
flowchart TD
  ...
  %% Transaction attributes (commonly used as columns)
  T --> Date[Date]
  T --> Label[Label]

  %% Position / Lot structure
  Pos --> Lot[Lot]
  Lot --> Units[Units]
  Units --> UnitsCur[Currency]
  Lot --> Cost[Cost]
  Cost --> CostCur[Currency]
  Cost --> CostNum[Number]

  %% Price structure
  Price --> PriceCur[Currency]
...
This document attempts to provide a good high-level summary of the features supported in our query language. However, should you find you need more information, you may take a look at the original proposal, or consult the source code in the beanquery repository. In particular, the parser tests will provide insight into the specifics of the syntax, and the query env tests will shed some light on the supported data columns and functions. Feel free to rummage in the source code and ask questions on the mailing-list.
